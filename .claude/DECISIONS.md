...
- **The model stops emitting native tool calls mid-conversation.**
  On 2026-07-19 a status turn died with `chars=0`: `status.summary`
  was called correctly, then three consecutive responses came back
  `finish="stop"` with zero tool calls, including the literal text
  `reply.commit({"spoken":"…","artifacts":[]})` — correct tool,
  correct arguments, wrong channel — with `tool_choice="required"`
  set. Same shape as the D-023 triage failure.

  **Not yet diagnosed.** Temperature was 0.7 on every phase and
  unconfigurable (a zero value silently became 0.7), which is a poor
  setting for protocol-strict tool calling and is now `AI_TEMPERATURE`
  defaulting to 0.1. Whether that is the cause is unmeasured.

  Also unknown: **which model this actually is.** `AI_MODEL` is
  `local-model`, LM Studio's passthrough identifier, so the logs do
  not record it. Native tool-calling quality varies enormously
  between local models and chat templates.

  Ordered next steps: (1) re-run at 0.1 and count violations,
  (2) record the real model name in the boot log, (3) if it persists,
  test whether LM Studio honours `tool_choice` for this model at all,
  and whether a grammar/structured-output constraint is available.
  Only if all of that fails is a text-encoded-call parser worth
  revisiting — it was built and reverted on 2026-07-19 as premature.
